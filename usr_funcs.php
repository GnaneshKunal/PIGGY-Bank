...
function do_index_menu(){
  ...
  <center>
   <nav class="navbar navbar-light bg-faded">
    ...
    <div class="nav navbar-nav"> <a class="nav-item nav-link active" href="index.php">Login</a> <a class="nav-item nav-link active" href="index.php?task=register">Register</a> </div>
   </nav>
  </center>
  <?php
}
...
function dep_get_cus_deposit($id){
  global $mysqli;
  $sql="select a.deposit_a,a.deposit_number,a.customer_id,a.amount,a.account_number,a.date from deposit as a,customer as b,employee as c where c.employee_id=$id and c.dependent_name=b.customer_name and a.deposit_a=1";
  try{
    if($result=$mysqli->query($sql)){
    if($result->num_rows>0){
      if($row=$result->fetch_array()){
